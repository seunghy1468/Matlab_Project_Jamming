...
switch type_jam
    ...
    case 32, str_jam='Velocity Gate Pull Off'; % 빨라짐
    case 31, str_jam='Velocity Gate Pull In';  % 느려짐
...
uicontrol(p1,'Style','text','Units','normalized',...
               ...
               'Position',[.15 .6 .7 .3]);
uicontrol(p1,'Style','text','Units','normalized',...
               'String','위협체: Monopulse CW Radar','FontSize',10,...
...
subplot(231) %%%%%%%%%%%%
    ...
    if rflag==0 && signal_miss==0 % && only_ang_miss==0
        ...
        plot([0 max_range*cos((az_beam-theta3/2)*pi/180)],...
            [0 max_range*sin((az_beam-theta3/2)*pi/180)],'Color','blue');
        est_bin=az_beam-theta3/2;
        est_bin_el=el_beam-theta3/2;
        if est_bin<0
            est_bin=est_bin+360;
        elseif est_bin>360
            est_bin=est_bin-360;
...
elseif only_ang_miss==1
    ...
    [azim,est_az ; elev,est_el ; vel,est_vel]
