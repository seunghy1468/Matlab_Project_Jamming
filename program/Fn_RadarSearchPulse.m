...
%     est_fd=10^6*((idx_f-1)/(tt*FBank))-f_IF*1000; % [Hz] estimated Doppler freq
%     est_vel=c*(10^2)*est_fd/(2*direction*f_RF); % [m/s] estimated velocity
% %     ff_spec=10^6/(tt*FBank); % [Hz] sampling interval in freq
% %     one_vel=c*(10^2)*ff_spec/(2*direction*f_RF) % [m/s] velocity per sample
% end
...
%     case 61, str_jam='RGPO + VGPO'; % 빠르게 멀어짐
%     case 62, str_jam='RGPO + VGPI'; % 느리게 멀어짐
...
uicontrol(p1,'Style','text','Units','normalized',...
               ...
               'Position',[.15 .6 .7 .3]);
uicontrol(p1,'Style','text','Units','normalized',...
               'String','위협체: Search Radar (Pulse Type)','FontSize',10,...
               'Position',[.1 .35 .8 .2],'HorizontalAlignment','left');
uicontrol(p1,'Style','text','Units','normalized',...
...
if rflag==0 && signal_miss==0, bflag=1; end    
pre_est_R=est_R;
...
subplot(231) %%%%%%%%%%%%
        ...
        if rflag==0 && signal_miss==0 % && only_ang_miss==0
            plot([0 max_range*cos(est_bin_az*pi/180)],...
                [0 max_range*sin(est_bin_az*pi/180)],'Color','blue');
            plot([0 max_range*cos((est_bin_az+theta3)*pi/180)],...
                [0 max_range*sin((est_bin_az+theta3)*pi/180)],'Color','blue');    
            if est_bin_az<180
                if est_bin_az+theta3<=180 %%% mod(180,theta3)~=0 일때 필요함
                    cx=r2*cos((est_bin_az+theta3)*pi/180):0.01:r2*cos(est_bin_az*pi/180);
                    plot(cx,sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                else
                    cx=-r2:0.01:r2*cos(est_bin_az*pi/180);
                    plot(cx,sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                    cx=-r2:0.01:r2*cos((est_bin_az+theta3)*pi/180);
                    plot(cx,-sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                end
            else % est_bin>=180
                if est_bin_az+theta3<=360 %%% mod(180,theta3)~=0 일때 필요함
                    cx=r2*cos(est_bin_az*pi/180):0.01:r2*cos((est_bin_az+theta3)*pi/180);
                    plot(cx,-sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                else
                    cx=r2*cos(est_bin_az*pi/180):0.01:r2;
                    plot(cx,-sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                    cx=r2*cos((est_bin_az+theta3)*pi/180):0.01:r2;
                    plot(cx,sqrt(r2^2-cx.^2),'Color','blue','LineWidth',3);
                end
            end
        else
...
subplot(232) %%%%%%%%%%%%
        if rflag==0 && signal_miss==0 % && only_ang_miss==0
            Fn_PlotAngScope(theta3,est_bin_az,est_bin_el,signal_miss,1);
            plot(azim,elev,'rx','LineWidth',2);
        else
            Fn_PlotAngScope(theta3,est_bin_az,est_bin_el,0,0);
        end
...
subplot(234),
    if rflag==1 || only_ang_miss==1
        ...
        plot(t_rcv, graph_signal), grid on; hold on;
        axis([start_rcv end_rcv -rmm rmm]);
        plot(t_rcv, [zeros(1,idx-1),0.5,zeros(1,Nrcvp-idx)],...
            'LineWidth',2);
    end
    title('Received Signal'), xlabel('time (\mus)');
    if rflag==0 && signal_miss==0 && only_ang_miss==0
        text(start_rcv+10, rmm-0.2, ['R = ',num2str(R),' km']);
        text(start_rcv+10, rmm-0.4, ['R_{est} = ',num2str(est_R),' km']);
...
if rflag==1
    str_est_R='탐색 범위 밖'; str_err_R='';
    str_est_az='탐색 범위 밖'; str_err_az='';
    str_est_el='탐색 범위 밖'; str_err_el='';
    str_est_V='추정할 수 없음'; str_err_V='';        
else
    str_est_R=[num2str(est_R),' km']; str_err_R=[num2str(R-est_R),' km'];
    R_sqerr_sum=R_sqerr_sum+(R-est_R)^2;
    str_est_az=[num2str(est_bin_az+theta3/2),' deg']; str_err_az='';
    str_est_el=[num2str(est_bin_el+theta3/2),' deg']; str_err_el='';
    str_est_V='추정할 수 없음'; str_err_V='';    
end
uicontrol(p2,'Style','text','Units','normalized',...
...
subplot(232) %%%%%%%%%%%%
        if rflag==0 && signal_miss==0 % && only_ang_miss==0
            Fn_PlotAngScope(theta3,(k-1)*theta3,est_bin_el,signal_miss,0);
%             plot(azim,elev,'rx','LineWidth',2);
        else
            Fn_PlotAngScope(theta3,(k-1)*theta3,est_bin_el,1,0);
        end
...
if rflag==1
    ...
    str_est_V='추정할 수 없음'; str_err_V='';   
elseif bflag==1
    str_est_R=[num2str(pre_est_R),' km']; str_err_R=[num2str(R-pre_est_R),' km'];
    R_sqerr_sum=R_sqerr_sum+(R-pre_est_R)^2;
    str_est_az=[num2str(est_bin_az+theta3/2),' deg']; str_err_az='';
    str_est_el=[num2str(est_bin_el+theta3/2),' deg']; str_err_el='';
    str_est_V='추정할 수 없음'; str_err_V='';    
else
    str_est_R='스캔되지 않음'; str_err_R='';
    str_est_az='스캔되지 않음'; str_err_az='';
    str_est_el='스캔되지 않음'; str_err_el='';
    str_est_V='추정할 수 없음'; str_err_V='';        
end
uicontrol(p2,'Style','text','Units','normalized',...
               'String',['추정: ',str_est_R],'FontSize',10,...
...
uicontrol(p3,'Style','text','Units','normalized',...
               'String',['rms 방위각 오차: '],'FontSize',10,...
               'Position',[.1 .35 .7 .15],'HorizontalAlignment','left');  
uicontrol(p3,'Style','text','Units','normalized',...
               'String',['rms 고도각 오차: '],'FontSize',10,...
               'Position',[.1 .2 .7 .15],'HorizontalAlignment','left');   
uicontrol(p3,'Style','text','Units','normalized',...
               'String',['rms 속도 오차: '],'FontSize',10,...
