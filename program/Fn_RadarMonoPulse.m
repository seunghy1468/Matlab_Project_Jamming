function Fn_RadarMonoPulse()
%%% 2018.04.03.16:20 최종 수정

load each_sim_2018.mat
ccc=1;
...
%% *** Jamming ***
if ang_miss_cnt>=re_tracking, jflag(k)=0; end
if type_jam>0
    ...
    elseif jflag(k)<=dur_jam && jflag(k)>0 % do jamming
        ...
        switch type_jam
            ...
            case 41 %% azim (+) angle jamming
                a_vel_jam=0.5; % [deg/s]
                azi_jam=(dur_jam-jflag(k))*t_intv*a_vel_jam;
            case 42 %% azim (-) angle jamming
                a_vel_jam=-0.5; % [deg/s]
                azi_jam=(dur_jam-jflag(k))*t_intv*a_vel_jam;   
            case 61 %% VGPO
                v_jam=vel-acc_jam*(dur_jam-jflag(k))*t_intv; % [m/s]
...
elseif ang_miss_cnt>=re_tracking % start re-tracking
    az_beam=azim-rand(1,1)*2; % initial beam center
    el_beam=elev-rand(1,1)*2; % initial beam center   
    ang_miss_cnt=0;
end
...
if jflag(k)>0 && jflag(k)<=dur_jam && (type_jam==41 || type_jam==42) % angle jamming
    V=Fn_AnttenaGain(g0, theta3, theta_s, azi_angle+azi_jam, elv_angle);
else
...
if signal_miss==0
    ...
    if ang_miss_cnt==0 && (abs((az_beam+hat_azi)-azim) <= theta3 ...
...
%%-- Range Tracking
if (type_radar==2 || type_radar==3) && signal_miss==0 && only_ang_miss==0
    if k==1, r_gate=idx:idx+Ntx-1; end
    if rflag==0 && jflag(k)<=dur_jam && jflag(k)>0
        r_gate=idx:idx+Ntx-1;
    else
        e_gate=r_gate(1):r_gate(Ntx/2);
        l_gate=r_gate(Ntx/2+1):r_gate(end);
        r_gate=r_gate+round(sum(abs(gc_signal(l_gate)))-...
            sum(abs(gc_signal(e_gate))));
...
switch type_jam
    ...
    case 41, str_jam='Phase Pull out';   % (+) 각도
    case 42, str_jam='Phase Pull in';   % (-) 각도
...
uicontrol(p1,'Style','text','Units','normalized',...
               ...
               'Position',[.15 .6 .7 .3]);
uicontrol(p1,'Style','text','Units','normalized',...
               'String','위협체: Monopulse Radar','FontSize',10,...
...
subplot(231) %%%%%%%%%%%%
    ...
    if type_jam>0 && jflag(k)>0 && jflag(k)<=dur_jam && ang_miss_cnt<3
...
subplot(232) %%%%%%%%%%%%
    ...
    if rflag==0 && signal_miss==0 && only_ang_miss==0
        ...
        plot(az_beam+hat_azi, el_beam+hat_elv, 'bo',...
            'MarkerFace','b','MarkerSize',7),
...
subplot(234),
    ...
    title('Gain Controlled Signal'), xlabel('time (\mus)');
    if rflag==0 && signal_miss==0 && only_ang_miss==0
...
%     if rflag==0 && signal_miss==0 && only_ang_miss==0 && type_radar>2
...
%     [R,est_R ; azim,est_az ; elev,est_el]
    R_sqerr_sum=R_sqerr_sum+(R-est_R)^2;
    Az_sqerr_sum=Az_sqerr_sum+(azim-est_az)^2;
    Ev_sqerr_sum=Ev_sqerr_sum+(elev-est_el)^2;
    str_est_R=[num2str(est_R),' km']; str_err_R=[num2str(R-est_R),' km'];
    str_est_az=[num2str(est_az),' deg']; str_err_az=[num2str(azim-est_az),' deg'];
    str_est_el=[num2str(est_el),' deg']; str_err_el=[num2str(elev-est_el),' deg'];
    str_est_V='추정할 수 없음'; str_err_V='없음';   
end
uicontrol(p2,'Style','text','Units','normalized',...
